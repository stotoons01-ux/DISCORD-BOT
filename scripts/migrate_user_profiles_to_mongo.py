"""Migrate `user_profiles.json` -> MongoDB (idempotent upserts).

Usage:
  python migrate_user_profiles_to_mongo.py [--dry-run]
"""
import json
...
def main(dry_run: bool = True):
    ...
    print(f"Found {len(data)} user profiles in {FILE}")
    ...
    for uid, profile in data.items():
        ok = UserProfilesAdapter.set(str(uid), profile)
        if ok:
            count += 1

    print(f"Migrated {count} user profiles to Mongo")
    return 0
